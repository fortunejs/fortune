...
exports.inputRecord = function (type, record) {
  var hasTransform = !this.transforms ||
    (this.transforms[type] && this.transforms[type].input)
  ...
  for (i = fieldsArray.length; i--;) {
    ...
    result[field] = hasTransform ? clone(value) : value
  }
...
exports.outputRecord = function (type, record) {
  var hasTransform = !this.transforms ||
    (this.transforms[type] && this.transforms[type].output)
  var recordTypes = this.recordTypes
  var primaryKey = this.keys.primary
  var typeKey = this.keys.type
  var isArrayKey = this.keys.isArray
  ...
  for (i = fieldsArray.length; i--;) {
    ...
    if (fieldIsDenormalized) {
      Object.defineProperty(result, field, {
        ...
        value: hasTransform ? clone(value) : value
      })
      continue
    }

    if (field in record) result[field] = hasTransform ?
      clone(value) : value
  }
