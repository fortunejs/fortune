    ...
    .then(function () {
      return new Promise(function (resolve, reject) {
        ...
        function listener (event) {
          ...
          self.worker.removeEventListener(listener)

          types = Object.keys(event.data.result)
          for (i = 0, j = types.length; i < j; i++) {
            type = types[i]
            self.db[type] = reduce(event.data.result[type], reducer, {})
          }

          return resolve()

          function reducer (hash, record) {
  ...
  IndexedDBAdapter.prototype.update = function (type, updates) {
    ...
    .then(function (count) {
      return count ? new Promise(function (resolve, reject) {
        ...
        self.worker.postMessage({
          id: id, type: 'update',
          payload: { type: type, records: records }
        })
