...
export default Adapter => class IndexedDBAdapter extends Adapter {
  ...
  connect () {
    ...
    const reconnect = (version, resolve, reject) => {
      const request = indexedDB.open(name, (version || 0) + 1)
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = event => {
        ...
        return needUpgrade ? reconnect(db.version, resolve, reject) : resolve()
      }
    })
  }
    ...
    return new Promise((resolve, reject) => {
      for (let update of updates) {
        const getRequest = objectStore.get(
          type + delimiter + update[keys.primary])
