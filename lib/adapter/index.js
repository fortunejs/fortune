 ...
 * **MUST** be a non-trivial primitive type.
 */
export default class Adapter {
   ...
   * fields it should be `[]` (empty array). Note that not all fields in the
   * schema may be enumerable, such as denormalized inverse fields, so it is
   * necessary to iterate over fields using `Object.getOwnPropertyNames`.
   *
   * @param {String} type
   ...
   * ```
   *
   * The syntax of the `query` object depends on the specific adapter, and
   * it allows for arbitrary querying which may be an expensive operation.
   ...
   *   // schema enforcement and referential integrity. Use at your own risk.
   *   operate: { ... }
   * }
   * ```
   *
