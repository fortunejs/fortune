 ...
 * returned by the adapter must have the primary key `id`. The primary key
 * **MUST** be a non-falsy primitive type.
 */
export default class Adapter {
  ...
  /**
   * Find records by IDs and options. If IDs is undefined or empty, it should
   * try to return all records. The format of the options may be as follows:
   *
   * ```js
   * {
   *   filter: { ... },
   ...
   * ```
   *
   * Things to consider: a `set` operation should not be applied to an array,
   * and `push` and `pull` should not be applied to non-arrays. The same value
   * in the same field should not exist in both `push` and `pull`.
   *
   * @param {String} type
  ...
  /**
   * Delete records by IDs, or delete the entire collection if IDs are undefined
   * or empty. Success does not need to resolve to any value.
   *
   * @param {String} type
   * @param {*[]} [ids]
   * @param {Object} [options]
  ...
  /**
   * Apply operations on a record, then return the record. This method is
   * optional to implement.
   *
   * @param {Object} record
   * @param {Object} operate - The `operate` field on an `update` object.
   * @return {Object}
   */
