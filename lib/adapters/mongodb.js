    ...
    model.find(match).exec(function(error,resources){
      ...
      RSVP.all(_.map(resources, function(resource){
        return new Promise(function(resolve, reject) {
          ...
          model.findOneAndUpdate(match, {$set: {_links: links}, deletedAt: new Date()}, { new: true }, function (error) {
            if (error) {
              reject(error);
            } else {
              resolve(resource);
...
function getInverseReferences(model, modifiedRefs){
  return adapter._getReferences(model, modifiedRefs).filter(function(ref){
    return !!ref.inverse;
  });
}
adapter._getReferences = getReferences;
adapter._getInverseReferences = getInverseReferences;
adapter._getAllReferences = getReferences;
