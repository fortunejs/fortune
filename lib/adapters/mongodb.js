...
function shouldUpsert(model, resource, opts) {
  opts = opts || {};
  ...
  if (status && matches.length) {
    ...
    if (/\./.test(matchKey)) {
      var parts = matchKey.split("."),
        ...
        if (_.has(resource, first) && _.has(resource[first], second)) {
          match[first] = {};
