  ...
  return new Promise(function(resolve, reject) {
    ...
    relatedModel.update(match, dissociate, function(error) {
      ...
      relatedModel.update(match, associate, {multi: true}, function(error) {
        if(error) return reject(error);
        unbindRedundant(model, relatedModel, resource, reference.path, field.path, function(err){
...
function unbindRedundant(model, relatedModel, resource, refFrom, refTo, callback){
  if (!resource[refFrom]) return callback();
  ...
  relatedModel.findOne(match).lean(true).exec(function(err, matching){
    if (err) return reject(err);
    if (matching){
      var selfMatch = {
        $and: [
      ...
      model.update(selfMatch, unbind, {multi: true}, function(err){
        callback(err);
      });
    }else{
      callback();
