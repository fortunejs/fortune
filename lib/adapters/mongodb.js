...
adapter._updateOneToOne = function(relatedModel, resource, reference, field) {
  return new RSVP.Promise(function(resolve, reject) {
    // Dissociation
    var query = {$unset: {}};
    query.$unset[field.path] = 1;
    relatedModel.where(field.path, resource.id).update(query).exec(function(error) {
      if(error) {
        return reject();
      }
      // Association
      var query = {$set: {}};
      query.$set[field.path] = resource.id;
...
adapter._updateOneToMany = function(relatedModel, resource, reference, field) {
  return new RSVP.Promise(function(resolve, reject) {
    // Dissociation
    var query = {$pull: {}};
    query.$pull[field.path] = resource.id;
    relatedModel.where(field.path, resource.id).update(query).exec(function(error) {
      if(error) {
        return reject();
      }
      // Association
      var query = {$addToSet: {}};
      query.$addToSet[field.path] = resource.id;
      relatedModel.findByIdAndUpdate(
        resource[reference.path],
        query,
        resolve
...
adapter._updateManyToOne = function(relatedModel, resource, reference, field) {
  return new RSVP.Promise(function(resolve, reject) {
    ...
    relatedModel.where(field.path, resource.id).update(query).exec(function(error) {
      ...
      var query = {$set: {}};
      query.$set[field.path] = resource.id;
      var idSelector = resource[reference.path] ?
        {_id: {$in: resource[reference.path]}} : {};

      relatedModel.find(idSelector)
        .update(query).exec(function(error) {
          if(error) return reject();
          resolve();
        });
    });
