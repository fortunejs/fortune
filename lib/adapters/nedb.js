...
adapter.model = function(name, schema) {
  if(schema) {
    var dbPath = this._options.path + this._options.db
      , filePath = dbPath + '/' + inflect.pluralize(name)
      , options = {
      filename: filePath,
      autoload: true
    };
...
adapter.find = function(model, query) {
  ...
  return new RSVP.Promise(function(resolve, reject) {
    model.findOne(query, function(error, resource) {
      if(error || !resource) return reject(error);
      resolve(_this._project(_this._deserialize(model, resource)), fields);
    });
  });
};
...
adapter.findMany = function(model, query, limit) {
  ...
  if(_.isArray(query) && query.length) {
    ...
    dbQuery = (query.ids && query.ids.length) ? {_id: {$in: query.ids}} : {};
    fields = query.fields || [];
  }
  model = typeof model === 'string' ? this._models[model] : model;
  ...
  return new RSVP.Promise(function(resolve, reject) {
    model.find(dbQuery, function(error, resources) {
      ...
      resources = resources.map(function(resource) {
        return _this._project(_this._deserialize(model, resource), fields);
      });
      resolve(resources);
...
/**
 * Creates a projection of json
 */

adapter._project = function(json, fields){
  return (fields && fields.length) ? _.pick(json, fields) : json;
};
