...
Fortune.prototype.constructor = function (options) {
  ...
  var self = this
  var recordTypes = {}, transforms = {}, flows = {}
   ...
   * @type {Adapter}
   */
  var adapter = new AdapterSingleton({
   ...
   * @type {Serializer}
   */
  var serializer = new SerializerSingleton({
...
Fortune.prototype.transform = function (type, input, output) {
  ...
  if (![ input, output ].some(isFunctionType))
...
// A bit of closure optimization and avoiding `Function.prototype.bind`.
function bindMiddleware (scope, middleware) {
  return function (x) {
    return middleware.call(scope, x)
  }
}


function isFunctionType (x) {
