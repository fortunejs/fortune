...
/**
 * This is the default export of the `fortune` module. The Fortune class has
 * a few static properties attached to it that may be useful to access:
 *
 * - `adapters`: included adapters, currently defaulting to NeDB for the
 * server, and IndexedDB for the browser (with Web Storage as a fallback).
 ...
 */
export default class Fortune extends EventEmitter {
  ...
  constructor (options = {}) {
    ...
    /**
     * A hash that maps strings to symbols. Available are: `find`, `create`,
     * `update`, and `delete`.
     *
     * @type {Object}
     */
    ...
    Object.defineProperties(this, {
      ...
      // Singleton instances.
      adapter: { value: this.adapter, enumerable: true },
      serializer: { value: this.serializer, enumerable: true },

      // Dispatch.
      change: { value: this.change, enumerable: true },
      methods: { value: methods, enumerable: true },
      flows: { value: Object.keys(methods).reduce((hash, method) =>
        Object.assign(hash, {
  ...
  dispatch () {
    if (this.connectionStatus !== 2)
      throw new Error(`Instance must be connected before dispatching.`)
