 ...
 *   ```
 *
 * - `types`: an object keyed by name, valued by fields object. Here are some
...
Fortune.prototype.constructor = function (options) {
  var self = this
  var i, adapter, serializer, method, stack, flows, type, types, transforms

  if (typeof options.types !== 'object' || !Object.keys(options.types).length)
...
Fortune.prototype.request = function (options, a, b) {
  ...
  if (self.connectionStatus === 1)
    return Promise.reject(new Error(
      'Instance is in the process of connecting.'))

  if (self.connectionStatus === 0) {
    console.warn( // eslint-disable-line
      'The instance was not connected prior to making the first request. ' +
      'Call the "connect" method before trying to request.')
