 ...
 *   // adapte, which may introspect the field definition.
 ...
 *   ```
 *
 * - `transforms`: keyed by type name, valued by an array containing a function
 *   for transforming input and output at indices `0` and `1` respectively.
 *
  ...
  for (type in transforms) {
    ...
    if (!Array.isArray(transforms[type]))
      throw new TypeError('Transform value for " + type + " type ' +
        'must be an array containing at most two functions.')
  }
 ...
 *
 * - `ids`: An array of IDs, used only for `find` and `delete` methods, ignored
 *   on other methods.
 *
 ...
 *
 * - `payload`: Payload of the request, used only for `create` and `update`,
 *   ignored on other methods.
 *
 * The response object contains the following keys:
 *
 * - `meta`: Meta-info of the response.
 * - `records`: Records returned, without included records.
 * - `include`: A hash mapping types to included records.
 *
