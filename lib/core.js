...
export default class Core {
   ...
   *   adapter: {
   *     // May be a string (module name or path), object, or class that
   *     // implements the adapter methods.
   *     // Default: 'nedb'
   *     type: 'nedb',
   *
   ...
   *   // Default: { 'application/vnd.api+json': { type: 'json-api', options: {} } }
   *   serializer: {
   *     'application/vnd.api+json': {
   *       // May be a string (module name or path), object, or class that
   *       // implements the serializer methods.
   *       type: 'json-api',
   *
   ...
   * ```
   *
   * Options are frozen once they are set, to prevent edge cases.
   *
  ...
  constructor (options = {}) {
    ...
    this.options = Object.freeze(setDefaults(options));
   ...
   *
   * Schemas are frozen immediately after they are defined.
   *
  ...
  resource (name) {
    ...
    if (!schemas.hasOwnProperty(name)) {
      // Parse and freeze the input.
      schemas[name] = Object.freeze(Parser.apply(null, arguments));
    } else {
   ...
   */
  listen () {
    if (!Object.isFrozen(this.schemas)) {
