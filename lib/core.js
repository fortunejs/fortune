...
export default class Core {
   ...
   *   // field on an entity that is unique and identifies it.
   *   // Default: 'id'
   *   primaryKey: 'id',
   *
   *   // A function that accepts no parameters, and returns an unique
   *   // string, number, or buffer. This function must be synchronous
   *   // to encourage speed and discourage lookups for implementing
   *   // incremental IDs. If this is not specified, then ID
   *   // generation will be left to the adapter.
   *   // Default: undefined
  ...
  resource (type, schema, options) {
    ...
    if (!(type in this.schemas)) {
      this.schemas[type] = Parser(type, schema, options);
    } else {
