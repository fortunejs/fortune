...
export default function (context) {
  ...
  return adapter.find(type, updates.map(update => update.id), options)
  ...
  .then(t => {
    ...
    for (let update of transformedUpdates) dropFields(update, fields)

    return transaction.update(type, transformedUpdates, options)
  })
  ...
  .then(() => {
    ...
    for (let update of transformedUpdates)
      for (let field of links) {
        ...
        if (update.replace && field in update.replace) {
          ...
          for (let id of ids)
            if (!arrayProxy.includes(record[field], id))
            update.push[field].push(id)

          for (let id of record[field])
            if (!arrayProxy.includes(ids, id))
    ...
    return Promise.all(Object.keys(relatedUpdates)
      .map(type => relatedUpdates[type].length ?
        transaction.update(type, relatedUpdates[type], options) :
        Promise.resolve()))
  })
  ...
  .then(() => {
    ...
    for (let linkedType of Object.keys(relatedUpdates)) {
      ...
      else eventData[methods.update][type] = [ ...union(
