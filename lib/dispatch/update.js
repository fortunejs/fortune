...
function applyUpdate (record, fields, update) {
  ...
  for (let field in update.replace)
    clone[field] = update.replace[field]

  for (let field in update.push) {
    const value = update.push[field]
    clone[field] = clone[field].slice()
    if (Array.isArray(value)) clone[field].push(...value)
    else clone[field].push(value)
  }

  for (let field in update.pull) {
    const value = update.pull[field]
    clone[field] = clone[field].slice().filter(exclude.bind(null,
