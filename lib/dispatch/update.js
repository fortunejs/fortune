...
export default function (context) {
  ...
  .then(t => {
    ...
    return transaction.update(type, transformedUpdates.map(update =>
...
function dropFields (update, fields) {
  const result = { [keys.primary]: update[keys.primary] }

  if ('replace' in update) {
    result.replace = {}
    for (let field in update.replace)
      if (field in fields)
        result.replace[field] = update.replace[field]
  }
  ...
  if ('push' in update) {
    ...
    for (let field in update.push)
      if (!(field in fields))
        result.push[field] = update.push[field]
  }
  ...
  if ('pull' in update) {
    ...
    for (let field in update.pull)
      if (!(field in fields))
        result.pull[field] = update.pull[field]
  }

  if ('operate' in update)
    result.operate = update.operate
