...
const idempotentAction = events.find
...
export default class Dispatcher extends EventEmitter {
   ...
   * ```js
   * {
   *   // The action is a Symbol type, keyed under `events` and may be one of
   *   // `find`, `create`, `update`, or `delete`. To implement a custom
   *   // action, define a new flow under `flows`, and define middleware
...
function dispatchRequest (context, ...args) {
  ...
  const { processRequest, processResponse } = this.serializer
  ...
  .then(context => runFlow.call(this, context.request.action, context))
 ...
 * @return {Promise}
 */
function runFlow (action, context) {
