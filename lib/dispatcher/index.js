...
const idempotentMethod = methods.find
...
export function dispatch (options, ...args) {
  ...
  .then(context => {
    ...
    if (method !== idempotentMethod) {
      if (!type) throw new errors.MethodError(`The type is unspecified.`)
      if (!(type in schemas)) throw new errors.NotFoundError(
  ...
  .catch(error => {
    ...
    try {
      ...
      context = Serializer.prototype.showError(context, error)
    }
