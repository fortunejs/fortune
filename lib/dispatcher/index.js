...
const defaultFlows = {
  create: ['doCreate', 'fetchInclude'],
  find: ['showIndex', 'fetchPrimary', 'fetchInclude'],
  update: ['doUpdate', 'fetchInclude'],
  delete: ['doDelete']
};
...
export default class Dispatcher extends events.EventEmitter {
  ...
  constructor () {
    ...
    Object.defineProperty(this, '_changeEvent', {
      value: changeEvent
    });
  }
...
function dispatchRequest (context, ...args) {
  ...
  let runFlow = (action, context) => {
    // Block invalid action.
    if (!flows.hasOwnProperty(action))
      throw new errors.MethodError(`The action type "${action}" ` +
        `is unrecognized.`);

    let flow = flows[action].slice();
    flow.unshift('requestStart');
    flow.push('requestEnd');

    return flow.reduce((chain, middleware) =>
  ...
  return Promise.resolve(this.serializer.processRequest(context, ...args))
    .then(context =>
      runFlow(context.request.action, context)
