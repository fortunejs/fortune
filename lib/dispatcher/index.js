...
const methods = {
  [events.find]: [ 'doFind', 'fetchInclude', 'endRequest' ],
...
export function dispatch (options, ...args) {
  ...
  return new Promise((resolve, reject) =>
    ...
    // Can't use `Promise.resolve` here, because we want the `catch` handler
    // to catch the error.
    new Promise(resolve => resolve(processRequest(context, ...args)))
    ...
    .then(context => {
      ...
      return runMethod.call(this, context.request.method, context)
    })
    ...
    .then(context => {
      ...
      if (request.method === events.create)
        Wrapper = success.Created

      if (!response.payload)
        Wrapper = success.Empty

      return resolve(new Wrapper(response))
    })
    .catch(error => {
      try {
