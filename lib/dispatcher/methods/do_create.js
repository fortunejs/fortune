...
export default function (context) {
  ...
  if (isRelated) {
    ...
    if (entities.filter(entity => inverseField in entity).length)
  ...
  return Promise.all(entities.map(entity => {
    ...
    if (createdEntities.filter(entity => !(primaryKey in entity)).length)
        ...
        if (field in entity && !!inverseField) {
          ...
          linkedIds.forEach(id => {
            ...
            if (id in idCache[linkedType]) {
              update = updates[linkedType].filter(
                update => update.id === id)[0];
            } else {
              update = { id };
