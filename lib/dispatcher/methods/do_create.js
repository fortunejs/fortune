...
import arrayFind from '../../common/array_find';
...
export default function (context) {
  ...
  if (isRelated) {
    ...
    if (arrayFind(entities, entity => inverseField in entity))
  ...
  return Promise.all(entities.map(entity => {
    ...
    if (arrayFind(createdEntities, entity => !(primaryKey in entity)))
        ...
        if (field in entity && !!inverseField) {
          ...
          linkedIds.forEach(id => {
            ...
            if (id in idCache[linkedType]) {
              update = arrayFind(updates[linkedType],
                update => update.id === id);
            } else {
              update = { id: id };
