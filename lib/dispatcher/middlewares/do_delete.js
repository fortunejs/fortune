...
export default function (context) {
  ...
  return this.adapter.find(type, ids).then(records =>
    'before' in (this.transforms[type] || {}) ? records.map(record =>
      this.transforms[type].before(context, record)) : records)
      ...
      links.forEach(field => {
        ...
        if (field in record && inverseField) {
          ...
          linkedIds.forEach(id => {
            ...
            if (linkedIsArray) {
              update.remove[inverseField] = update.remove[inverseField] || [];
              update.remove[inverseField].push(record[primaryKey]);
            } else {
              update.remove[inverseField] = true;
            }
          });
        }
      });
