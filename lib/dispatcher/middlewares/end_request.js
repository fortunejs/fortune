...
export default function (context) {
  ...
  return Promise.all(records.map(record => Promise.resolve(
    'output' in (this.transforms[type] || {}) ?
      this.transforms[type].output(context, record) : record)))
  ...
  .then(updatedRecords => {
    ...
    return Promise.all(includeTypes.map(includeType =>
      Promise.all(include[includeType].map(record => Promise.resolve(
        'output' in (this.transforms[includeType] || {}) ?
          this.transforms[includeType].output(context, record) : record)))
