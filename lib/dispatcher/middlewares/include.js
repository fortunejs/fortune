...
export default function (context) {
  const { type, ids, include } = context.request
  const { records } = context.response
  ...
  return Promise.all(include.map(fields => new Promise((resolve, reject) => {
    ...
    return fields.reduce((records, field) => records.then(cursor => {
      ...
      currentType = recordTypes[currentType][field][keys.link]
  ...
  .then(containers => {
    const include = containers.reduce((include, container) => {
      ...
      if (!include[container.type]) include[container.type] = []

      // Only include unique IDs per type.
      if (!idCache[container.type]) idCache[container.type] = new Set()
