...
Fortune.prototype.resource = function(name, schema, options, schemaCallback) {
  ...
  this._resources[name] = { actions: actionsObj, name: name, route: _this.inflect.pluralize(name), schema: schema, modelOptions: modelOptions, hooksOptions: hooksOptions, policy: policyOptions, validation: validation};
  ...
  this.beforeWrite(function(req, res) {
    var validateErrors = _this._validateResource(name, req.body);
 ...
 * @returns {*}
 */
Fortune.prototype._validateResource = function (model, data) {
  var that = this;
  var returnError = [];
  _.each(data[inflect.pluralize(model)], function(resource) {
    var toValidate = [];
    _.each(resource, function(value, key) {
      if(!_.isUndefined(that._resources[model].schema[key])) {
        var constraints = that._resources[model].schema[key].validation;
        if(typeof constraints == "string") {
          constraints = [constraints];
        }
        if(Array.isArray(constraints)) {
          _.each(constraints, function(constraint) {
            if(!_.isUndefined(that._validators[constraint])) {
              var rule = {}; rule[key] = that._validators[constraint];
              toValidate.push(rule);
            } else {
              console.warn("Undefined validator used: " + constraint);
            }
          });
        } else if(!_.isUndefined(constraints)) {
          var rule = {}; rule[key] = constraints;
          toValidate.push(rule);
        }
      }
    });
    for(var i in toValidate) {
      var validateError = validate(resource, toValidate[i]);
      if(!_.isUndefined(validateError)) {
        returnError.push(validateError[Object.keys(validateError)[0]]);
      }
    }
  });
  return returnError;
