...
Fortune.prototype._exposeResourceDefinitions = function() {
  ...
  this.router.get("/resources", function(req,res){
    res.write(JSON.stringify({resources: _.map(_this._resources, function(md, name){
      ...
      return _.extend({name: name}, md, { schema: schema });
    }, {})}));
    res.end();
  }); 
};
...
Fortune.prototype.beforeAllWrite = GlobalHook('_before', 'write');
