...
Fortune.prototype._init = function (options) {
  ...
  if( _.has( options, 'customInstrumentor' ) &&
      ( !_.has( options.customInstrumentor, 'methods' ) ||
        !_.has( options.customInstrumentor.methods, 'createTracer' ) ||
        !_.isFunction( options.customInstrumentor.methods.createTracer )
      )
  ){
  ...
  if( !_.has( options.customInstrumentor.options, 'tracePrefix' ) ){
    options.customInstrumentor.options.tracePrefix = 'Fortune: ';
  }
...
Fortune.prototype.resource = function(name, schema, options, schemaCallback) {
  var _this = this,
      ...
      authMethods = null,
      customInstrumentor = this.options.customInstrumentor;
  ...
  this.adapter.awaitConnection().then(
    customInstrumentor.methods.createTracer( customInstrumentor.options.tracePrefix + ' await connection',
    function () {
