...
Fortune.prototype._exposeResourceDefinitions = function() {
  ...
  this.router.get(this.options.serviceName ? "/resources/" + this.options.serviceName : "/resources", function(req,res){
    res.write(JSON.stringify({resources: _.map(_this._resources, function(md, name){
      ...
      return _.extend({name: name}, md, { schema: schema }, _this.options.serviceName ? {service: _this.options.serviceName} : {});
    }, {})}));
    res.end();
  }); 
};
...
Fortune.prototype.beforeAllRW = function(hooksArray){
  hooks.registerGlobalHook('_before', 'read', hooksArray);
  hooks.registerGlobalHook('_before', 'write', hooksArray);
  return this;
};
Fortune.prototype.afterAllRW = function(hooksArray){
  hooks.registerGlobalHook('_after', 'read', hooksArray);
  hooks.registerGlobalHook('_after', 'write', hooksArray);
  return this;
};
...
function GlobalHook(time, type){
  return function(hooksArray){
    hooks.registerGlobalHook(time, type, hooksArray);
    return this;
  }
}
