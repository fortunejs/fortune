 ...
 *
 * ### Database setup
 * - `adapter`: may be either "mongodb", "mysql", "psql", or "sqlite". Default: "mongodb".
 ...
 *
 * *Note: in order to use the relational database adapters, you must also install the node.js drivers from `npm`: `mysql` for MySQL, `pg` for Postgres, and `sqlite3` for SQLite.*
 */
...
/**
 * Scrub invalid keys from a schema.
 *
 * @api private
 * @param {Object} schema
 * @return {Object}
...
Fortune.prototype.listen = function() {
  this.express.listen.apply(this.express, arguments);
...
Fortune.prototype._removeRoutes = function(name, methods, routes) {
  ...
  this.adapter.awaitConnection().then(function() {
    (methods || []).forEach(function(verb) {
      var paths = _this.express.routes[verb];
