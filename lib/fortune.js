...
Fortune.prototype.resource = function(name, schema, options, schemaCallback) {
  ...
  this.adapter.awaitConnection().then(
    ...
    try {
      ...
      var mongoschema = _.clone(schema);

      _.each(_.keys(customTypes), function(key) {
        mongoschema[key] = mongoose.Schema.Types.Mixed;
      });

      mongoschema = _this.adapter.schema(name, mongoschema, options, schemaCallback);

      // Pass any upsertKeys to the schema
      mongoschema.upsertKeys = upsertKeys || [];

      if(name == "request-leg") {
        console.log(mongoschema);
      }

      _this._route(name, _this.adapter.model(name, mongoschema, modelOptions), _this._resources, inflect, _this._querytree, _this._metadataProviders);
