...
Fortune.prototype.resource = function(name, schema, options, schemaCallback) {
  ...
  var mongooseSchema = _.clone(schema);
  _.each(_.keys(customTypes), function(key) {
    // Assign mongoose.Schema.Mixed to all custom data types
    mongooseSchema[key] = mongoose.Schema.Mixed;
  });

  this.adapter.awaitConnection().then(
    instrumentor.createTracer( customInstrumentorObj.options.tracePrefix + ' await connection',
    function () {
    mongooseSchema = _this._preprocessSchema(monoggseSchema);

    // Store a copy of the input.
    _this._mongooseSchema[name] = _.clone(mongooseSchema);
    try {
      mongooseSchema = _this.adapter.schema(name, monogooseSchema, options, schemaCallback);

      // Pass any upsertKeys to the schema
      mongooseSchema.upsertKeys = upsertKeys || [];

      _this._route(name, _this.adapter.model(name, mongooseSchema,modelOptions), _this._resources, inflect, _this._querytree, _this._metadataProviders);
