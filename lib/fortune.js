...
/**
 * Lets you get a currently available fortune-created fortune-route.
 * - NB: Fortune creates routes asyncronously, and this method only returns a route if it has already been created.
 */
Fortune.prototype.getCreatedRoute=function(name){
  return createdRoutes[name];
}

Fortune.prototype.getOnRouteCreated=function(name){
  return onRouteCreatedHandlers[name];
}
/**
 * Lets you set a callback for a specific routeCreated event.
 * * @param {String} name - the name of the route you want the callback to apply to
 * * @param {Function} fn - the callback function
 * * @param {Object} context - the fn's context
 * - The callback will be passed the newly created route.
 */
Fortune.prototype.setOnRouteCreated=function(name,fn,context){
  onRouteCreatedHandlers[name]=_.bind(fn,context);
}
/**
...
Fortune.prototype.resource = function (name, schema, options) {
  ...
  this.adapter.awaitConnection().then(function () {
    ...
    try {
      ...
      var onRouteCreated= _this.getOnRouteCreated(name);
      onRouteCreated && onRouteCreated(createdRoutes[name]);
    } catch(error) {
