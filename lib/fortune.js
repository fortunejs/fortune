...
/*  // Collect custom types map for the resource
  var customTypesMap = _.reduce(Object.keys(schema), mapCustomTypes(schema), []);
  function mapCustomTypes(schema, prefix){
    return function(memo, key){
      if((typeof schema[key]).toLowerCase() === "string") {
        var customType = _this._customTypes[schema[key]];
        if(!customType) throw new Error("Custom Type \"" + schema[key] + "\" is not defined");
        memo.push({
          path: prefix ? [prefix, key].join('.') : key,
          type: customType
        });
      } else if (_.isArray(schema[key]) && _.isObject(schema[key][0]) && !schema[key][0].ref) {
        memo = memo.concat(_.reduce(Object.keys(schema[key][0]), mapCustomTypes(schema[key][0], key), []));
      }
      return memo;
    }
  }

  _.each(customTypesMap, function(type){
