...
/**
 * Each instance of Fortune keeps track of three singletons: the adapter,
 * serializer, and dispatcher, which are accessible at `this.adapter`,
 * `this.serializer`, and `this.dispatcher` respectively. The Fortune class
...
export default class Fortune {
  ...
  constructor (options = {}) {
    ...
    const dispatcher = new Dispatcher({
      adapter, serializer, schemas, transforms
    })
  ...
  transform (type, input, output) {
    ...
    if (!arrayProxy.find([ input, output ], fn => typeof fn === 'function'))
...
defineArguments(Fortune, { adapters, serializers, net, errors })
