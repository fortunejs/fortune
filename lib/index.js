...
export default class Fortune {
   ...
   *   // to be created, and returns an unique string or buffer.
  ...
  resource (type, schema, options) {
    if (typeof type !== 'string' || !type.length)
      return console.warn(`Resource must have a name.`);
    ...
    if (type in this.schemas)
      return console.warn(`The resource "${type}" was already defined.`);

    this.schemas[type] = Schema.Parser(schema, options, type);
   ...
   * two arguments, the internal `context` object and a single `entity`.
   * The before transform may take an additional argument, `update`, if the
   * request is to update an entity.
   *
   * A transform has two parts, before it is written to, and after it is read
   * from the data store, either is optional. It must return the second
  ...
  transform (type, before, after) {
    ...
    if (!(type in this.schemas))
      return console.warn(`Attempted to define transform on "${type}" ` +
   ...
   */
  request () {
    if (this._initializationStatus !== 2)
      return console.warn(`The \`init\` method must be done first.`);
