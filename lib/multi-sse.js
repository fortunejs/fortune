...
MultiSSE.prototype.init = function(harvesterApp, resourceName) {
    ...
    var routePrefix = (resourceName) ? '/' + inflect.pluralize(resourceName) : '';

    console.log(routePrefix)
    harvesterApp.router.get(this.options.baseUrl + routePrefix + '/changes/stream', this.requestValidationMiddleware.bind(this), tinySSE.head(), tinySSE.ticker({seconds: 3}), this.handler.bind(this));
};

MultiSSE.prototype.requestValidationMiddleware = function (req, res, next) {
    console.log('requestValidationMiddleware')
    routeNames = req.query.resources ? req.query.resources.split(',') : [];

    if (this.resourceName) {
        routeNames = this.resourceName;
    }
...
MultiSSE.prototype.handleError = function(err, res, docStream) {
...
MultiSSE.prototype.getFilters = function(req) {
