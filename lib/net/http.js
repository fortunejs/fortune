 ...
 * object is mutated with implicit headers, so that the consumer only has to end
 * the response. The only integration it has with Fortune is in doing content
 * negotiation to determine which serializer to use, and mapping the response
 * from the `dispatch` method on to the response object.
 *
...
export default function http (app, request, response) {
  return new Promise(resolve => {
    ...
    return app.dispatch(options, request, response)
