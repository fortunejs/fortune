...
module.exports = function (HttpSerializer) {
  ...
  function (contextResponse, request, response) {
    ...
    function renderSidebar (type) {
      ...
      var match = map(
        ...
        function (field) {
          return tag('input', {
            name: 'match.' + field,
            value: (request.meta.options.match &&
              request.meta.options.match[field]) || '',
            placeholder: field
          })
        })
      ...
      return [
        ...
        tag('form', {
          ...
          tag('h4', message('Include', language)),
          tag('div', {
            'class': 'input-group'
          }, include),
          tag('h4', message('Fields', language)),
          tag('div', {
            'class': 'input-group'
          }, fields),
          tag('h4', message('Sort', language)),
          tag('div', {
            'class': 'input-group'
          }, sort),
          tag('h4', message('Match', language)),
          tag('div', {
            'class': 'input-group'
          }, match),
          tag('h4', message('Pagination', language)),
          tag('div', {
            'class': 'input-group'
          }, [
            tag('input', {
    ...
    function renderInputGroup (type, key, record) {
      ...
      var fields = fieldIsArray ? map((record[key] || [])
        .concat(Array.apply(null, Array(3))), castToString) :
        [ castToString(record[key]) ]
