  ...
  function (contextResponse, request, response) {
    ...
    function renderIndex () {
      ...
      page.push(tag('div', {
        ...
        tag('div', { 'class': 'container' }, tag('div', [
          tag('h1', name),
        ...
        (function () {
          ...
          for (type in recordTypes)
            for (field in recordTypes[type]) {
              ...
              if (linkKey in definition) {
                if (definition[linkKey] in seen[field]) continue
                seen[field][definition[linkKey]] = true
              }
              if (typeKey in definition) {
                if (definition[typeKey].name in seen[field]) continue
          ...
          for (field in documentation) {
            if (field in fields || field in recordTypes) continue
            fields[field] = []
          }
              ...
              tag('div', {
                ...
                fields[field].map(function (definition) {
                  return [
                    tag('link', {
                      'property': 'rdfs:range',
                      'typeof': definition[linkKey] ?
                        '/#' + definition[linkKey] :
                        xsdMap[definition[typeKey].name] ||
                        'xsd:simpleType'
                    }),
                    tag('span', { 'class': 'type' },
