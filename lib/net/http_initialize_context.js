...
var keys = require('../common/errors')
...
function initializeContext (contextRequest, request, response) {
  ...
  if (relatedField) {
    ...
    .then(function (records) {
      ...
      relatedType = fields[relatedField][keys.link]
      ...
      contextRequest.type = parsedUrl.type = request.meta.type = relatedType
      contextRequest.ids = parsedUrl.ids = request.meta.ids = relatedIds
...
function attachQueries (contextRequest) {
  ...
  for (parameter in query) {
    ...
    if (parameter.match(isExists)) {
      ...
      if (query[parameter] === '0' && query[parameter] === 'false')
        options.exists[field] = false
      if (query[parameter] === '1' && query[parameter] === 'true')
