...
var HttpJsonSerializer = require('./http_json_serializer')
 ...
 *   // JSON and form serializers if none are specified.
 *   serializers: [
 *     {
 *       type: HttpSerializerSubclass,
 *       options: { ... }
 *     }
 *   ],
 *   settings: {
...
function http (instance, options) {
  ...
  var i, j, type, CustomSerializer, mediaType,
  ...
  if (!('serializers' in options))
    options.serializers = [
      { type: HttpJsonSerializer },
      { type: HttpFormSerializer.formData },
      { type: HttpFormSerializer.formUrlEncoded }
    ]
  if (!('settings' in options)) options.settings = {}
  settings = options.settings

  for (i = 0, j = options.serializers.length; i < j; i++) {
    type = options.serializers[i].type

    if (typeof type !== 'function')
      throw new TypeError('The HTTP serializer type must be a function.')

    CustomSerializer = HttpSerializer.prototype.isPrototypeOf(type.prototype) ?
      type : type(HttpSerializer)
    ...
    serializers[mediaType] = new CustomSerializer({
      ...
      castValue: castValue,
      options: options.serializers[i].options || {},
  ...
  return function (request, response) {
    var encoding, payload, requestMethod, isProcessing, contextResponse
    ...
    .then(function (contextRequest) {
      ...
      .then(function () {
        return serializers[serializerInput].parsePayload(contextRequest)
      })
    ...
    .then(function (contextRequest) {
      requestMethod = contextRequest.method
    ...
    .then(function (result) {
      ...
      // This may be used internally by HTTP serializers.
      contextResponse.meta.requestMethod = requestMethod
    ...
    .catch(function (error) {
      ...
      .then(function () {
        if (!('payload' in error) &&
          ...
          error = assign(error.isInputError ?
        ...
        return !isProcessing && serializerOutput in serializers ?
          serializers[serializerOutput]
            .processResponse(error, request, response) :
          error
      })
      .then(function (error) {
...
http.JsonSerializer = HttpJsonSerializer
http.FormSerializer = HttpFormSerializer
