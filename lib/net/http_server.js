...
var statusMap = require('./http_status_map')
var instantiateSerializer = require('./http_instantiate_serializer')
...
function http (instance, options) {
  ...
  function end (contextResponse, request, response) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (response.statusCode === statusMap.get(responseClass.OK))
        response.statusCode = (statusMap.get(responseClass.Empty))
      payload = contextResponse.payload = ''
      return resolve()
    })
    .then(function () {
...
http.instantiateSerializer = instantiateSerializer
