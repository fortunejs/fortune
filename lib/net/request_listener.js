...
export default function (systemRequest, systemResponse) {
  ...
  systemRequest.on('end', () => {
    ...
    this.request(options, ...arguments).then(
      createResponse(response, systemResponse, defaultSuccessStatus),
      createResponse(response, systemResponse, defaultErrorStatus));
  });
}


function createResponse (response, systemResponse, defaultStatus) {
  return () => {
