...
function sendResponse (options, request, response) {
  ...
  if (payload) {
    ...
    if (request.method === 'POST')
      statusCode = statusMap.get('created');

  } else if (!statusCode)
    statusCode = statusMap.get('empty');

  if (request.headers.hasOwnProperty('origin'))
    Object.assign(this.meta, getCorsResponseHeaders());

  statusCode = (this instanceof Error ?
    statusMap.get(this.constructor) : statusCode) || options.defaultStatus;

  response.writeHead(statusCode, this.meta);
