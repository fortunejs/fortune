 ...
 * ```
 *
 * Handler functions are called within the context of the socket (`this` is
 * a socket instance) for each  connection.
 *
...
export default function websocket (instance, options, handlers = {}) {
  ...
  server.on('connection', socket => {
    socket.on('message', handlers.message.bind(socket))
    ...
    function changeHandler (event) {
      Promise.resolve(handlers.change.call(socket, event))
