 ...
 *
 * *Note that this is a single-threaded implementation*. In order to scale past
 * a single instance, inter-process communication is necessary. When one
 * instance emits a change event, all other instances must emit the same event.
 * This should be done in application code, since Fortune.js makes no
 * assumptions on how this should be done. Refer to the [Node.js documentation]
 * (https://nodejs.org/api/process.html) on IPC.
 *
