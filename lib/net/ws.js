...
/**
 * This function returns a WebSocket server that implements a wire protocol for
 * Fortune. The options are the same as those documented in the [`ws`
...
function ws (instance, change, options, callback) {
  ...
  server.on('connection', function (socket) {
    // Store connection state in a WeakMap.
    stateMap.set(socket, {})
    ...
    socket.on('message', function (data, flags) {
      ...
      if (!('state' in data) && !('request' in data))
        return sendError('Invalid request.', data.id)

      if ('state' in data)
        Promise.resolve(change(data.state))
        .then(function (state) {
          assign(stateMap.get(socket), state)
        }, function (error) {
    ...
    instance.on(changeEvent, function (changes) {
      return Promise.resolve(change(stateMap.get(socket), changes))
      .then(function (changes) {
        socket.send(msgpack.encode({ changes: changes }),
          { binary: true, mask: true })
      }, sendError)
    })
