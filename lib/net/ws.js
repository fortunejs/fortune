 ...
 * ```
 *
 * The returned `Server` object has an additional key `state`, which is a
  ...
  server.on('connection', function (socket) {
    ...
    socket.on('message', function (data, flags) {
      ...
      if ('state' in data)
        ...
        .then(function (state) {
          assign(server.state.get(socket), state)
    ...
    instance.on(changeEvent, function (changes) {
      return Promise.resolve(change(server.state.get(socket), changes))
