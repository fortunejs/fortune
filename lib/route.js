...
function route(name, model, resources, inflect, querytree, metadataProviders) {
  var _this = this
    ...
    , nr = this.options.customInstrumentor || {
 ...
 router.get(collectionRoute, getResources);
  ...
  function getResources(req, res) {
    ...
    nr.addCustomParameter('query', JSON.stringify( req.query ) );
    nr.addCustomParameter('userId', req.headers.user );

    function getInnr( req, res, callback ){
    var match = {};
    var ids = [];
    var projection = {};
    ...
    //run beforeRead
    beforeReadHook({}, req, res)
      .then(  nr.createTracer( model.modelName + '-parse', function(){
        if (select){
          projection = {
            select: select
          };
        ...
        return querytree.parse(model.modelName, _.extend(match, req.query.filter))
          .then( nr.createTracer( model.modelName + '-read', function(query){
          var queries = {};
    ...
    // run after read
      .then( nr.createTracer( model.modelName + '-response-preparation-1', function(result) {
        return RSVP.all(result.resources.map(
            nr.createTracer( model.modelName + '-response-preparation-2', function(resource) {
          return afterReadHook(resource, req, res);
        })))
        .then(nr.createTracer( model.modelName + '-response-preparation-3', function(r) {
          result.resources = r;
          return result;
        }));
      }))
    ...
    // send the response
      .then( nr.createTracer( model.modelName + '-sending-response', function(r) {
        var body = {};
    ...
    getInnr( req, res, nr.createTracer( model.modelName + '-entire-query',  function( err, data ){

      // fake slow fn to capture trace
      setTimeout( function(){
