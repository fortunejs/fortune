...
function route(name, model) {
    ...
    var methodNotAllowed = function (req, res) {
        sendError(req, res, jsonProblemError(405));
    };
    ...
    this.fnHandlers[collectionRoute]["post"] = function (req, res) {
        ...
        if (!mimeCheck(req.get('content-type'))) {
            return sendError(req, res, jsonProblemError(412));
        }
        ...
        adapter.findMany(model, ids)
            ...
            .then(function (resources) {
                if (resources.length) {
                    ...
                    sendError(req, res, jsonProblemError(404));
                }
            })
            // send the response
            .then(function (resources) {
    ...
    this.fnHandlers[individualRoute]["put"] = function (req, res) {
        ...
        if (!update) return sendError(req, res, jsonProblemError(400, 'incoming body payload is missing a primary resource collection'));
