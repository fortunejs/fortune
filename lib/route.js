...
function route(name, model) {
  ...
  var beforeTransform = function(model, resource, request) {
    ...
    return new RSVP.Promise(function(resolve) {
      ...
      resolve(_this._before[model].call(resource, request));
    });
  };
  ...
  var afterTransform = function(model, resource, request) {
    ...
    return new RSVP.Promise(function(resolve) {
      ...
      resolve(_this._after[model].call(resource, request));
    });
  };
  ...
  router.post(collectionRoute, function(req, res) {
    ...
    .then(function(resources) {
      ...
      if(!!baseUrl) {
        ...
        location += collection + '/' + resources[0].id;
   ...
   */
  router.get(individualRoute + '/:key', function(req, res) {
  ...
  router.delete(individualRoute, function(req, res) {
    ...
    // do before transform
    .then(function(resource) {
      return beforeTransform(resource);
    }, function(error) {
      sendError(req, res, 404, error);
    })
