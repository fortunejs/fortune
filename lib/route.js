...
function route(name, model, schema, routeOptions) {
    ...
    function resourceToBodyValidationDef(schema, idRequired) {
        var idDescriptor = {id: Joi.string().guid().description('id')};

        var schemaClone = _.clone(schema);
        var schemaWithId = _.merge(schemaClone, idRequired ? {id: idDescriptor.id.required()} : idDescriptor);
        ...
        var schemaWithIdAndlinks = _.set(schemaWithId, 'links', linksWithJoiDef);
        var schemaWithPrefixIdAndLinks = _.set({}, collection,  Joi.array().items(Joi.object(schemaWithIdAndlinks)));
        var finalSchema = Joi.object().keys(schemaWithPrefixIdAndLinks);

        return finalSchema;
    }

    var postBodyValidationDef = resourceToBodyValidationDef(schema, false);
    ...
    this.fnHandlers[collectionRoute]["post"] = function (req, res) {
        ...
        Validation.validate(req, postBodyValidationDef, {}, {}, {})
            .then(function (errors) {
                if (errors) throw new JSONAPI_Error({status: 422, details: errors})
    ...
    var putBodyValidationDef = resourceToBodyValidationDef(schema, true);
    ...
    this.fnHandlers[individualRoute]["put"] = function (req, res) {
        ...
        Validation.validate(req, putBodyValidationDef, {}, {}, {})
            .then(function (errors) {
                if (errors) throw new JSONAPI_Error({status: 422, details: errors})
                return true;
            })
