...
function route(name, model, resources, inflect) {
  ...
  function createResource(req, res){
    ...
    if (req.body.linked){
      ...
      .then(function(transformed){
        //Check if any before returned false
        if (hooksFailed(
        ...
        var matchedMainResources = _.clone(transformed.mainResources);
        _.each(transformed.linkedResources, function (linkedResources, key) {
          ...
          //Find related paths in mainResources
          var refPaths = _.filter(refsFromMain, function(ref){
            return ref.type === key;
          });
          //Map linked ids to main resources fields
          //As linkedResources is always of different type it's safe to overwrite createdMainResource
    ...
    function mix(main, linked, linkedModel){
      if (!main){
        ...
        return createLinked(linkedModel, linked);
      }else{
  ...
  function findResources(type, ids, projection) {
    var adapter = this.adapter,
        model = adapter.model(inflect.singularize(type));
    return adapter.findMany(model, ids, projection);
  }
  ...
  function getLinked(fetchedIds, resources, schema, key, req, res) {
    ...
    var mixed = _.clone(fetchedIds[type]);
    mixed.unshift(ids);
    ids = _.without.apply(null, mixed);
    ...
    if (ids.length > 0) {
      ...
      return beforeReadHook(inflect.singularize(type), {}, req, res)
        ...
        .then(function(){
          ...
          return findResources.call(_this,type,ids, projection)
        }, function(err){
          sendError(req, res, 500, err);
        })
