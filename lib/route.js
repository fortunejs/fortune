...
function route(name, model, schema, routeOptions) {
    ...
    this.fnHandlers[collectionRoute]["post"] = function (req, res) {
        ...
        function resourceToValidationDef(schema) {
            ...
            var validationDef = _.set({}, collection, Joi.array().items(_.merge(schema, idDescriptor)));
            return _.set(validationDef, collection+ '.links', _.mapValues(schema.links, function (val) {
                if (_.isArray(val)) {
