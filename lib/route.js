...
function route(name, model, resources, inflect) {
  ...
  function createResource(req, res) {
    ...
    // handle creation of linked resources
      .then(function (resources) {
        ...
        if (!resources) return RSVP.resolve();
  ...
  function appendLinked(linkpath, body, resources, schema, inclusions, req, res) { 
            ...
            .then(function(result) {
              ...
              if (result) {
                if (config[key].__includeInBody) {
                  ...
                  body.linked[result.type] = mergeByIds(body.linked[result.type], result.resources);
    ...
    function mergeByIds(existent, additional){
      var ids = [];
      return _.reject(_.map(existent.concat(additional), function(item){
