...
function route(name, model) {
    ...
    var constructErrorWithProblem = function (status, detail) {
        var problem = {problem: {httpStatus: status, problemType: 'about:blank', title: errorMessages[status]}};
    ...
    var sendError = function (req, res, error) {
        ...
        function normaliseError(error) {
                ...
                error :
            {
                ...
                title: errorMessages[500],
                detail: error ? error.toString() : ''
            };
        }

        res.set('Content-Type', 'problem+json');
        var normalisedError = normaliseError(error);
        res.send(normalisedError.httpStatus, stringify(normalisedError));
    };
    ...
    var methodNotAllowed = function (req, res) {
        sendError(req, res, constructErrorWithProblem(405));
    };
    ...
    this.fnHandlers[collectionRoute]["post"] = function (req, res) {
        ...
        if (!mimeCheck(req.get('content-type'))) {
            return sendError(req, res, constructErrorWithProblem(412));
        }
        ...
        adapter.findMany(model, ids)
            ...
            .then(function (resources) {
                if (resources.length) {
                    ...
                    sendError(req, res, constructErrorWithProblem(404));
                }
            }, function (error) {
                sendError(req, res, error);
            })
    ...
    this.fnHandlers[individualRoute]["put"] = function (req, res) {
        ...
        if (!update) return sendError(req, res, constructErrorWithProblem(400, 'incoming body payload is missing a primary resource collection'));
    ...
    this.fnHandlers[individualRoute]["delete"] = function (req, res) {
        ...
        adapter.find(model, id)
            ...
            // resource not found
            function (error) {
                var normalisedError = (!!error) ? error : constructErrorWithProblem(404);
                sendError(req, res, normalisedError);
            });
    };
