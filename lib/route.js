...
function route(name, model, resources, inflect, querytree, metadataProviders) {
  var _this = this
    ...
    , instrumentor = this.options.customInstrumentor
 ...
 router.get(collectionRoute, getResources);
  ...
  function getResources(req, res) {
    ...
    var tracePrefix =  instrumentor.options.tracePrefix + '/' + model.modelName;

    if(req.query.ids) match.id = {$in: req.query.ids.split(',')};
    //run beforeRead
    beforeReadHook({}, req, res)
      .then(  instrumentor.methods.createTracer( tracePrefix + ' parse', function(){
        if (select){
          projection = {
            select: select
          };
        ...
        return querytree.parse(model.modelName, _.extend(match, req.query.filter))
          .then( instrumentor.methods.createTracer( tracePrefix + ' read', function(query){
          var queries = {};
      ...
      // run after read
      .then( instrumentor.methods.createTracer( tracePrefix + ' response-preparation', function(result) {
      ...
      // send the response
      .then( instrumentor.methods.createTracer( tracePrefix + ' sending-response', function(r) {
        var body = {};
