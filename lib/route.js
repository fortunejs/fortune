...
function route(name, model, schema, routeOptions) {
    ...
    function resourceToBodyValidationDef(schema, put) {
        ...
        var schemaWithId = _.merge(schemaClone, put? _.set(idDescriptor, 'id', idDescriptor.id.forbidden()) : idDescriptor);
    ...
    this.fnHandlers[collectionRoute]["post"] = function (req, res) {
        ...
        Validation.validate(req, postBodyValidationDef, {}, {}, {})
            .then(function (causes) {
                if (causes) throw new JSONAPI_Error({status: 422, meta: {causes: causes}})
