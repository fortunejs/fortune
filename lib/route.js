...
function route(name, model, resources, inflect, querytree, metadataProviders) {
 ...
 router.get(collectionRoute, getResources);
  ...
  function getResources(req, res) {
    ...
    //run beforeRead
    beforeReadHook({}, req, res)
      .then( nr.createTracer('Custom-beforeRead', function(){
        if (select){
          projection = {
            select: select
          };
        ...
        return querytree.parse(model.modelName, _.extend(match, req.query.filter))
          .then( nr.createTracer( 'Custom-parse ' + model.modelName, function(query){
          var queries = {};
    ...
    // run after read
      .then( nr.createTracer('Custom-read ' + model.modelName, function(result) {
        nr.addCustomParameter('userId', 'abcdefghjklmnop');
        return RSVP.all(result.resources.map(
            nr.createTracer('Custom-resource-maps', function(resource) {
          return afterReadHook(resource, req, res);
        })))
        .then(nr.createTracer('Custom-post-resource-map', function(r) {
          result.resources = r;
          return result;
        }));
      }))
    ...
    // send the response
      .then( nr.createTracer('Custom-afterRead ' + model.modelName, function(r) {
        var body = {};
    ...
    getInnr( req, res, nr.createTracer( "Custom-" + tracername , function( err, data ){
      setTimeout( function(){
