...
function route(name, model, schema, routeOptions) {
    ...
    this.fnHandlers[collectionRoute]["post"] = function (req, res) {
        ...
        function resourceToValidationDef(schema) {
            var idDescriptor = {id: Joi.string().guid().description('id')};
            var schemaWithId = _.merge(schema, idDescriptor);
            var linksWithJoiDef = _.mapValues(schema.links, function (val) {
                if (_.isArray(val)) {
            ...
            return _.set({}, collection, Joi.array().items(Joi.object(schemaWithIdAndlinks)));
        }

        var bodyValidationDef = resourceToValidationDef(schema);
        Validation.validate(req, bodyValidationDef, {}, {}, {})
