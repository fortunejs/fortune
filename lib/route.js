...
function route(name, model, resources, inflect, querytree, metadataProviders) {
  ...
  function Hook(time, type){
    return function(model, resource, request, response, isNew){
      ...
      var tracePrefix = instrumentorObj.options.tracePrefix + time +  '-hooks';
  ...
  function updateResource(req, res) {
    ...
    .then(function( returnedDocument ){
      ...
      try {
        ...
        req.body.forEach(function (operation) {
          ...
          if (operation.op === 'replace') {
            ...
            if (field.length > 1 && field[0] !== "links") {
              ...
              //~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*
              // this allows the removal of values from nested arrays, but not
              // objects. May be worth considering implementing similar for
              // nested arrays of objects but maybe not yet
              //~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*~*
              ...
              if( !nestedObjects ){
                // TODO can use fieldName below maybe?
                path[pullKey][field[field.length - 1]] = path[pullKey][field[field.length - 1]] || [];
                path[pullKey][field[field.length - 1]].push(value);
              }
              else{
                path[ pullKey ][ fieldName ] = { _id: { $in: [] } };
                path[ '$push' ] = {};
                path[ '$push' ][ "_" + fieldName ] = { $each: [] };

                var aliasName = path[ pullKey ][ fieldName ]._id;
                var otherAliasName = path[ '$push' ][ "_" + fieldName ];

                // populate the array of ids to remove aswell as the documets to
                // add to the _deleted array
                ...
                if( subdocsToRemove.length > 0 ){
                  ...
                  aliasName.$in = aliasName.$in.concat( idsToRemove );
                  otherAliasName.$each = otherAliasName.$each.concat( subdocsToRemove );
                }
              }
            }
          }
        ...
        console.log( '_~_~_~_~_~_~update~_~_~_~_~_~_~' );
