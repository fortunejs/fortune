...
function route(name, model, schema, routeOptions) {
    ...
    this.fnHandlers[collectionRoute]["post"] = function (req, res) {
        ...
        if (!_.isEmpty(details)) throw new JSONAPI_Error({status: 400, detail: 'validation failed on incoming request', meta: {details: details}});
    ...
    this.fnHandlers[individualRoute]["put"] = function (req, res) {
        ...
        if (!_.isEmpty(details))
            throw new JSONAPI_Error({status: 400, detail: 'validation failed on incoming request', meta: {details: details}});
