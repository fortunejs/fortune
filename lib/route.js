...
function route(name, model) {
  ...
  router.get(collectionRoute, function(req, res) {
    var schema = _this._schema[name];
    var query = {};

    // check for request query params that are attriutes in our collection
    _.each(req.query, function(value,key) { if (key in schema) query[key] = value });

    if(typeof req.query.ids === 'string') query = req.query.ids.split(',');
    if(typeof req.query.ids === 'array') query = req.query.ids;
