...
function route(name, model) {
  ...
  var errorMessages = {
    ...
    412: 'Request header "Content-Type" must be "' + mimeTypes[0] + '".',
  ...
  var sendError = function(res, status, error) {
    ...
    res.set('Content-Type', mimeTypes[1]);
    res.send(status, str);
  };
  var sendResponse = function(res, status, object) {
    ...
    res.set('Content-Type', mimeTypes[0]);
    res.send(status, str);
  };
  ...
  express.post(collectionRoute, function(req, res) {
    ...
    if(mimeTypes.indexOf(req.get('Content-Type')) < 0) {
