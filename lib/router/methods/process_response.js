import { Enforcer as enforce } from '../../schema';
...
export default function (context) {
  ...
  return Promise.all(entities.map(entity => new Promise(resolve => {
    return new Promise(resolve => resolve('after' in (this.transforms[type] || {}) ?
      ...
      .then(entity => {
        ...
        entity = enforce(entity, this.schemas[type],
          Object.assign(this.options.schema, { output: true }));
      ...
      resolve(Promise.all(include[includeType].map(entity =>
        new Promise(resolve => {
          return new Promise(resolve => resolve(
            ...
            .then(entity => {
              // Enforce the schema after running transform.
              entity = enforce(entity, this.schemas[includeType],
                Object.assign(this.options.schema, { output: true }));
              return resolve(entity);
            });
      })))
    ...
    if (Object.keys(include).length) {
      ...
      context = this.serializer.showResource(context, entities, include);
    } else {
      delete context.response._include;
      context = this.serializer.showResource(context, entities);
    }
