...
/**
 * Throw errors for mismatched types on a record to be created, and drop
 * arbitrary fields that are not defined in the schema.
 *
 * @param {Object} record
 * @param {Object} schema
 * @return {Object}
  ...
  for (let key in record) {
    ...
    if (keys.type in schema[key])
      if (schema[key][keys.isArray]) {
        ...
        value.forEach(checkValue.bind(null, schema[key], key))
      }
    ...
    if (keys.link in schema[key])
      if (schema[key][keys.isArray]) {
        ...
        if (arrayProxy.find(value, findPrimary.bind(null, record)))
