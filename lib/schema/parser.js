...
export default function Parser (schema = {}, options = {}) {
  for (let key in schema) {
    ...
    if (typeof value === 'function' || typeof value === 'string') {
      ...
      console.warn(`The definition of schema field "${key}" is invalid.`);
    }

    if (keys.type in output && !output[keys.type]) {
      console.warn(`Invalid type for "${key}".`);
      delete output.type;
    }
  ...
  // Store an hash keyed by links.
  schema._links = Object.keys(schema).reduce((hash, field) => {
...
function coerceString (constructor) {
  ...
  if (typeof constructor === 'string') {
    if (~types.indexOf(constructor)) {
      ...
      console.warn(`The type "${constructor}" is unrecognized.`);
    }
    ...
    if (constructor === String) type = 'string';
      ...
      console.warn(`Unknown type for "${type}".`);
    }

  } else {
    console.warn(`Invalid type "${constructor}".`);
  }
...
function parseObject (value, key) {
  ...
  for (let attribute in value) {
    if (attribute === keys.link) {
      ...
      if (typeof value[attribute] === 'string' && value[attribute].length) {
        ...
        console.warn(`The "${attribute}" key on the schema field "${key}" ` +
      ...
      if (typeof value[attribute] === 'string' && value[attribute].length) {
        ...
        console.warn(`The "${attribute}" key on the schema field "${key}" must ` +
  ...
  if (keys.type in output && keys.link in output) {
    ...
    console.warn(`The field "${key}" can not have both of the keys "${keys.link}" ` +
  ...
  if (!output[keys.type] && !output[keys.link]) {
    console.warn(`The schema field "${key}" must contain either "${keys.type}" ` +
      `or "${keys.link}" keys.`);
    return {};
  }
...
function coerceSingular (value, output, type, key) {
  if (Array.isArray(value)) {
    if (!value.length) {
      console.warn(`Empty array in "${type}" of key "${key}".`);
      return {};
    }
    if (value.length > 1) {
      console.warn(`Only the first value of "${type}" will be used on key "${key}".`);
    }
    value = value[0];
