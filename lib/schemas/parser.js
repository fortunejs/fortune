...
function coerceString (constructor) {
  ...
  if (typeof constructor === 'string') {
    if (~types.indexOf(constructor)) {
      ...
      warn('The type "' + constructor + '" is unrecognized. Defaulting to "' + defaultType + '".');
      type = defaultType;
    }
    ...
    if (constructor === String) type = 'string';
      ...
      warn('Unknown type for ' + type + ', defaulting to "' + defaultType + '".');
      type = defaultType;
    }

  } else {
    warn('Invalid type "' + constructor + '". Defaulting to "' + defaultType + '".');
    type = defaultType;
  }
...
function parseObject (value, key) {
  ...
  if (typeof value === 'function') {
    ...
    warn('The schema field "' + key + '" must contain either "' + keys.type + '" or "' +
      keys.link + '" keys.');
    return {};
