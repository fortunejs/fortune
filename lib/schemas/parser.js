...
function parseObject (value, key) {
  ...
  if (Object.keys(output).length && value.hasOwnProperty(keys.validate)) {
    if (typeof value[keys.validate] === 'function') {
      output.validate = value[keys.validate];
    } else {
      warn('The "' + keys.validate + '" key on the field "' + key +
        '" must be valued as a function.');
    }
  }
