...
module.exports = function () {
   ...
   * since it tries to limit itself to JSON-serializable objects and does not
   * try to include any special keys in the payload. Notably, the `include`
   * option is missing.
   */
    ...
    for (i = parameters.length; i--;) {
      ...
      if (parameter.match(isMatch)) {
        ...
        options.match[field] = Array.isArray(value) ?
          map(value, curryCast(castValue, fieldType, options)) :
  ...
  JsonSerializer.prototype.showResponse =
    function (context, records, include) {
      ...
      return DefaultSerializer.prototype
        .showResponse.call(this, context, records, include)
    }
