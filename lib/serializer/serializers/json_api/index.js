...
import { mapRecord, mapId, castValue, initializeContext, processData,
  stringifyObject, getByteLength } from './helpers'
  ...
  class JsonApiSerializer extends Serializer {
    ...
    constructor () {
      ...
      const methodMap = {
        ...
        OPTIONS: context => this.showAllow(context, context.request.url)
      }
    ...
    showAllow (context, url) {
      ...
      switch (degree) {
        case 0: allow = [ 'GET' ]; break
        case 1: allow = [ 'GET', 'POST' ]; break
        case 2: allow = [ 'GET', 'PATCH', 'DELETE' ]; break
        case 3: allow = [ 'GET' ]; break
        case 4: allow = [ 'GET', 'POST', 'PATCH', 'DELETE' ]; break
        default: break
      }
    ...
    parseCreate (context) {
      ...
      if (ids)
        throw new errors.MethodError(`Can not create with ID in the route.`)
    ...
    showError (context, error) {
      ...
      if (error.constructor === MethodError)
        this.showAllow(context, context.request.url)
