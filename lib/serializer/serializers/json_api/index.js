...
// JSON API is an highly verbose yet also ambiguous specification. There are
// many trade-offs made in an attempt to cover everyone's use cases, such as
// linkage objects for polymorphism (niche use case) and relationship entities
// which are an entirely unnecessary complication. More importantly, it
// assumes tight coupling with the API consumer and the HTTP protocol, and it
// is not a hypermedia format, therefore not REST.
//
  ...
  class JsonApiSerializer extends Serializer {
    ...
    processRequest (context) {
      ...
      .then(payload => {
        ...
        const { uriTemplate, options, schemas, adapter, keys, errors } = this
    ...
    showResponse (context, records, include) {
      const { keys, events, errors, uriTemplate, options, schemas } = this
      ...
      if (ids && ids.length && method === events.find &&
      ...
      } else if (relatedField && method === events.find) {
        output[reservedKeys.links] = {
          [reservedKeys.self]: prefix + uriTemplate.fillFromObject({
