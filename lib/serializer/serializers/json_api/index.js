...
export default class jsonApiSerializer extends Serializer {
  ...
  processRequest (context) {
    ...
    // Se inflection options.
    if (!('_inflect' in this))
      this._inflect = Object.assign(defaults.inflect, this.options.inflect);
  ...
  processResponse (context) {
    ...
    context.response.meta['content-type'] =
      context.request.serializerOutput;
...
function mapEntity (type, entity) {
  ...
  entity.links = {
    self: prefix + this._uriTemplate.fillFromObject({
      type: this._inflect.type ? inflection.pluralize(type) : type,
      ids: id
    })
  };
