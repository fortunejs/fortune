  ...
  class JsonApiSerializer extends Serializer {
    ...
    processRequest (context) {
      ...
      if (query) {
        ...
        context.request.includeOptions = {}

        context.request.include = query.include ?
          query.include.split(',').map(i => i.split('.')) : []

        if (query.sort) {
          const sort = Array.isArray(query.sort) ? query.sort : [query.sort]

          context.request.options.sort = sort.reduce((sort, field) => {
        ...
        Object.keys(query).forEach(parameter => {
          if (parameter.match(/^fields/)) {
            ...
            if (sparseType === type)
              context.request.options.fields = fields
            else if (sparseType) {
              if (!(sparseType in context.request.includeOptions))
                context.request.includeOptions[sparseType] = {}

              context.request.includeOptions[sparseType].fields = fields
            }
          }
        })
      }
    ...
    processResponse (context) {
      ...
      if (options.outputBuffer && payload && typeof payload === 'object')
        if (Object.keys(payload).length) context.response.payload =
