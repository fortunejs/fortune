...
export default Serializer => {
  ...
  class JsonApiSerializer extends Serializer {
    ...
    processRequest (context) {
      ...
      // Inflect type name.
      if (options.inflect.type)
        uriObject.type = inflection.singularize(uriObject.type)

      context.request.action = determineAction.call(this, context, request)
      ...
      .then(records => {
        ...
        if (!relatedIds.length && context.request.action !== events.create)
...
function mapRecord (type, record) {
  ...
  record[specKeys.links] = {
    [specKeys.self]: prefix + uriTemplate.fillFromObject({
      type: options.inflect.type ? inflection.pluralize(type) : type,
      ids: id
    })
  }
  ...
  for (let field in record) {
    ...
    if (options.inflect.keys && !specKeys.hasOwnProperty(field)) {
