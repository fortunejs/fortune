...
import { mapRecord, mapId, castValue, initializeContext } from './helpers'
...
class JsonApiSerializer extends Serializer {
  ...
  showResponse (context, records, include) {
    const { keys, methods, uriTemplate, options, recordTypes,
  ...
  showRelationship (context, records) {
    ...
    const { keys, uriTemplate, options, recordTypes, methods,
    ...
    const identifiers = records.map(record => ({
      [reservedKeys.type]: options.inflectType ?
  ...
  parseCreate (context) {
    ...
    return data.map(record => {
      const clone = {}
      const recordType = options.inflectType ?
      ...
      if (reservedKeys.relationships in record)
        for (let field of Object.keys(record[reservedKeys.relationships])) {
          ...
          const relatedType = options.inflectType ?
  ...
  parseUpdate (context) {
    const { recordTypes, keys, options,
      errors: { MethodError, BadRequestError, ConflictError } } = this
    ...
    for (let update of data) {
      const replace = {}
      const updateType = options.inflectType ?
  ...
  updateRelationship (context) {
    const { recordTypes, keys, methods, options,
  ...
  showError (context, error) {
    const { errors: { nativeErrors, MethodError } } = this
