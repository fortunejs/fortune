...
import { mapRecord, mapId, initializeContext, under } from './helpers'
...
class JsonApiSerializer extends Serializer {
  ...
  parseCreate (context) {
    const { keys, recordTypes, options, options: { inflectType, inflectKeys },
    ...
    return data.map(record => {
      ...
      if (reservedKeys.attributes in record)
        for (let field in record[reservedKeys.attributes]) {
          ...
          clone[field] = castValue.call(this,
  ...
  parseUpdate (context) {
    const { recordTypes, keys, options, options: { inflectType, inflectKeys },
      errors: { MethodError, BadRequestError, ConflictError } } = this
    ...
    for (let update of data) {
      ...
      if (!includes(ids, update[reservedKeys.id]))
      ...
      if (reservedKeys.attributes in update)
        for (let field in update[reservedKeys.attributes]) {
          ...
          replace[field] = castValue.call(this,
