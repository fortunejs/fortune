...
export function showLinks (type) {
  ...
  for (let field in schema) {
    ...
    // Show link fields that are not denormalized fields.
    if (inverse && !schemas[link][inverse][keys.denormalizedInverse])
      output[field][reservedKeys.inverse] = fieldDefinition[keys.inverse]
  }
...
export function mapRecord (type, record) {
  ...
  record[reservedKeys.links] = {
    [reservedKeys.href]: prefix + uriTemplate.fillFromObject({
      type: inflectType ? inflection.pluralize(type) : type,
      ids: id
    })
  }
  ...
  for (let field of fields) {
    ...
    // Leave typed schema fields intact.
    if (fieldDefinition[keys.type]) continue
