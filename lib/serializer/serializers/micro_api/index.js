...
import { processData, initializeContext, showLinks, mapRecord,
...
export default Serializer => {
  ...
  class MicroApiSerializer extends Serializer {
    ...
    showResponse (context, records, include) {
      ...
      const { methods, errors, options } = this
      const { queries } = options
      const { method, type, ids, relatedField } = context.request
      ...
      if (!ids) output[reservedKeys.meta].count = records.count

      // For the find method, it may be helpful to show available queries.
      if (method === methods.find)
        output[reservedKeys.meta].queriesAllowed = [ ...queries ]
