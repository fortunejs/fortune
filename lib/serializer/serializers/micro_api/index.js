...
import { processData, initializeContext, showLinks, showQueries,
  ...
  encodeRoute, getByteLength } from './helpers'
  ...
  class MicroApiSerializer extends Serializer {
    ...
    showAllow (context) {
      ...
      switch (degree) {
        ...
        case 1: allow = [ 'GET', 'POST', 'PATCH', 'DELETE' ]; break
        case 2: allow = [ 'GET', 'PATCH', 'DELETE' ]; break
        case 3: allow = [ 'GET', 'POST', 'PATCH', 'DELETE' ]; break
        default: break
      }
    ...
    parseCreate (context) {
      ...
      const {
        type, ids, payload, relatedField, relatedType, relatedIds
      } = context.request

      if (ids)
        throw new errors.MethodError(
      ...
      if (relatedField) {
        const field = recordTypes[relatedType]
          [relatedField][keys.inverse]
        const relatedArray = recordTypes[relatedType]
